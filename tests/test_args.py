...
class TestArgsWriter(ArgsTestCase):
	...
	def test_standard(self):
		args = self.returnArgs(["--start", "22", "index.txt", "input.pdf", "-o", "output.pdf"])
	...
	def test_dry(self):
		# test the dry flag
		args = self.returnArgs(["--start", "12", "index.txt", "input.pdf", "-o", "output.pdf", "--dry"])
		self.assertTrue(args.dry)
	def test_debug(self):
		# test the debug flag
		args = self.returnArgs(["--start", "12", "index.txt", "input.pdf", "-o", "output.pdf", "--debug"])
		self.assertTrue(args.debug)
	def test_missing_args(self):
		# test what happens when some required arguments are missing
		with self.assertRaises(SystemExit) as exc:
			self.returnArgs(["input.pdf", "--start", "12"])
		self.assertIn("required", exc.exception.code)
	def test_wrong_args(self):
		# test what happens when the arguments are not in the expected format
		with self.assertRaises(SystemExit) as exc:
			self.returnArgs(["--start", "ciao", "index.txt", "input.pdf", "-o", "output.pdf"])
		self.assertEqual(exc.exception.code, 2)
	def test_negative_start(self):
		# test what happens when the start argument is negative
		with self.assertRaises(SystemExit):
			self.returnArgs(["--start", "-1", "index.txt", "input.pdf", "-o", "output.pdf"])
